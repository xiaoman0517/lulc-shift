...
### 3. 配置持久化存储（可选：解决下载 / 地图不显示的根因）

Vercel 函数是无状态、按请求隔离、实例短暂的环境。本地运行依赖的「内存任务表 +
后台线程 + /tmp 文件」在线上多实例/回收后会失效（表现为下载提示“无法从网站上
获取文件”、地图瓦片加载不出来）。本项目内置两种应对方案：

**方案 A（零成本，无需任何付费服务）——结果预览内嵌返回**

任务完成时（结果文件还在执行实例的 /tmp 时）直接把 before/after/change 三幅栅格
降采样渲染成 PNG、以 data URL 随最后一次轮询一起返回；前端用 `L.imageOverlay`
直接上图，**不再发逐瓦片请求**，因此不依赖跨实例共享文件，地图叠加与卷帘在任何
Serverless 实例组合下都能显示（Vercel Hobby 免费即可用，无需 KV/Blob）。
代价是预览为降采样图，放大不如瓦片清晰；下载需在任务刚完成、实例尚温时进行。

**方案 B（体验更好，需免费额度服务）——Upstash Redis + Vercel Blob**
...
> 提示：方案 B 下 Vercel 会自动跳转使用 KV/Blob 持久化；若只是配置了 KV 而没配
> Blob（`redis-only`），结果文件仍写 /tmp，地图仍可能空白——方案 A 的预览图机制
> 恰好补上这一缺口。Upstash 免费额度（约 500MB / 每日 1 万次写）与 Vercel Blob
> Hobby 免费额度（5GB 存储 / 20GB/月带宽）对 Demo 均够用。
...
3. **多瓦片 mosaic**：已实现基础版——BBOX 同时命中多景（如跨 114°E 等 UTM 边界）时，引擎会取所有命中景裁剪拼接（以数据最多的一景为基准网格，最近邻填充空洞），避免大片 nodata
